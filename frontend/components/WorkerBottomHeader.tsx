...
const WorkerNavbar: React.FC = () => {
  const navigation = useNavigation<WorkerNavbarProps>();  // Type the navigation prop correctly
  const [activeTab, setActiveTab] = useState<string>('WorkerHome');

  const handleNavigate = (screenName: keyof WorkerStackParamList) => {
    console.log("Navigating to:", screenName);
    setActiveTab(screenName);
    navigation.navigate('WorkerRoutes', { screen: screenName });
    
    // navigation.navigate(screenName);  // Corrected navigate call
  };

  const handleLogout = async () => {
    await StorageUtils.removeAll();
    navigation.navigate('SignIn');  // Correctly navigate to the SignIn screen
  };
